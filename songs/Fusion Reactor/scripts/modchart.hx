...
function postCreate()
{
	...
	for (e in eventsToRemove) eventsShit.remove(e);
	...
	for (e in eventsShit) {
		...
		str += ' ease="' + e.ease + '"';
		str += ' type="tweenShaderProperty"';
		str += ' startValue="' + e.startValue + '"';
		str += ' name="' + e.name + '"';
		str += "/>\n";

		list += str;
	}
	File.saveContent("test.txt", list);
	*/
	//initModchart();
...
function ease(beat:Float, time:Float, easeFunc:String, data:String) {
	...
	for (i in 0...Math.floor(arguments.length/2))
	{
		...
		var n = split[0];
		var prop = "value";
		if (split.length > 1) prop = split[1];
		...
		if (eventsMap.exists(name)) {
			var lastEvent = eventsMap.get(name);
			e.startValue = lastEvent.value;
			if (lastEvent.step == e.step && lastEvent.time == 0) {
		...
		eventsShit.push(e);
		eventsMap.set(name, e);
	}
}

function set(beat:Float, data:String) {
